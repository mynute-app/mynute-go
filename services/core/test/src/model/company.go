	...
	for range 3 {
		...
		for _, service := range c.Services {
			if err := employee.AddService(200, service, &c.Owner.X_Auth_Token, nil); err != nil {
				return fmt.Errorf("failed to assign employee %s to service %s: %v", employee.Created.UserID, service.Created.Name, err)
			}
		}
		for _, branch := range c.Branches {
			if err := employee.AddBranch(200, branch, &c.Owner.X_Auth_Token, nil); err != nil {
				return fmt.Errorf("failed to assign employee %s to branch %s: %v", employee.Created.UserID, branch.Created.Name, err)
			}
		}
		employeeID := employee.Created.UserID
		...
		if err := employee.CreateWorkSchedule(200, employeeWorkSchedule, nil, nil); err != nil {
			return fmt.Errorf("failed to create work schedule for employee %s: %v", employee.Created.UserID, err)
		}
...
func (c *Company) GenerateEmployees(n int) error {
	...
	for i := range n {
		...
		if employee.Created.UserID == uuid.Nil {
		...
		for k := 0; k < numBranchesToAssign && k < len(shuffledIndices); k++ {
			...
			if err := employee.AddBranch(200, branchToAssign, &c.Owner.X_Auth_Token, nil); err != nil {
				return fmt.Errorf("failed to assign employee %d (%s, ID: %s) to branch (Name: %s, ID: %s): %v",
					i, employee.Created.UserID.String() /* TODO: Email is on User model */, employee.Created.UserID,
					branchToAssign.Created.Name, branchToAssign.Created.ID, err)
			}
			assignedCount++
		}
		if assignedCount == 0 && numBranchesToAssign > 0 {
			return fmt.Errorf("employee %s was intended to be assigned %d branches, but 0 were assigned. Valid branches: %d. This might be due to previous errors or lack of valid branches", employee.Created.UserID.String() /* TODO: Email is on User model */, numBranchesToAssign, len(validBranches))
		}
	}
	return nil
}
...
func (c *Company) RandomlyAssignServicesToEmployees() error {
	...
	for i, employee := range c.Employees {
		if employee.Created.UserID == uuid.Nil || employee.X_Auth_Token == "" {
		...
		for k := 0; k < numServicesToAssign && k < len(shuffledIndices); k++ {
			...
			if err := employee.AddService(200, serviceToAssign, nil, nil); err != nil {
				return fmt.Errorf("failed to assign service (Name: %s, ID: %s) to employee %d (%s, ID: %s): %v",
					...
					i, employee.Created.UserID.String() /* TODO: Email is on User model */, employee.Created.UserID, err)
			}
			assignedCount++
		}
		if assignedCount == 0 && numServicesToAssign > 0 {
			log.Printf("Warning: Employee %s was intended to be assigned %d services, but 0 were assigned. Valid services: %d.\n", employee.Created.UserID.String() /* TODO: Email is on User model */, numServicesToAssign, len(validServices))
		}
	}
	return nil
}
...
func (c *Company) RandomlyAssignWorkScheduleToEmployees() error {
	...
	for i, employee := range c.Employees {
		if employee.Created.UserID == uuid.Nil { // No need to check Auth_token here, Update uses Company token.
			return fmt.Errorf("skipping schedule assignment for employee %d (%s): ID nil", i, employee.Created.UserID.String() /* TODO: Email is on User model */)
		}
		...
		if len(validEmployeeBranches) != len(employee.Created.Branches) {
			return fmt.Errorf("employee %d (%s) has %d branches, but only %d valid branches found", i, employee.Created.UserID.String() /* TODO: Email is on User model */, len(employee.Created.Branches), len(validEmployeeBranches))
		} else if len(validEmployeeBranches) != len(employee.Branches) {
			return fmt.Errorf("employee %d (%s) has %d branches, but only %d valid branches found", i, employee.Created.UserID.String() /* TODO: Email is on User model */, len(employee.Branches), len(validEmployeeBranches))
		}

		EmployeeWorkSchedule, err := GenerateRandomEmployeeWorkRanges(employee.Branches, employee)
		if err != nil {
			return fmt.Errorf("failed to generate work schedule for employee %d (%s): %v", i, employee.Created.UserID.String() /* TODO: Email is on User model */, err)
		}
		...
		for {
			if len(EmployeeWorkSchedule.WorkRanges) > 0 {
				...
				return fmt.Errorf("failed to generate a valid work schedule for employee %d (%s) after %d attempts", i, employee.Created.UserID.String() /* TODO: Email is on User model */, scheduleCreationAttempts)
			}
		...
		if err := employee.CreateWorkSchedule(200, EmployeeWorkSchedule, nil, nil); err != nil {
			return fmt.Errorf("failed to create work schedule for employee %d (%s) via API: %v", i, employee.Created.UserID.String() /* TODO: Email is on User model */, err)
		}
	}
	return nil
}
...
func GenerateRandomEmployeeWorkRanges(validBranches []*Branch, employee *Employee) (DTO.CreateEmployeeWorkSchedule, error) {
	...
	for day := range 7 {
		if err := generateWorkRangeForDay(&EmployeeWorkSchedule.WorkRanges, validBranches, employee, model.Weekday(day), 0.9); err != nil {
			return DTO.CreateEmployeeWorkSchedule{}, fmt.Errorf("failed to generate work range for employee %d (%s) on day %d: %v", employee.Created.UserID, employee.Created.Name, day, err)
		}
	}
...
func generateWorkRangeForDay(ranges any, validBranches []*Branch, employee *Employee, weekday model.Weekday, workProbability float32) error {
	if employee == nil {
		...
		log.Printf("Generating work range for employee %d (%s) on weekday %d\n", employee.Created.UserID, employee.Created.UserID.String() /* TODO: Email is on User model */, weekday)
	}
	...
	for range numRanges {
		...
		case *[]DTO.CreateEmployeeWorkRange:
			log.Printf("Generated work range for employee (%s) on weekday %d: %s - %s\n", employee.Created.UserID, weekday, finalStartTimeStr, finalEndTimeStr)
...
func (c *Company) Create(status int) error {
	...
	// TODO: Password and Email are on User model in auth service, not on Employee
	// ownerPassword = ownerPswd
	// ownerEmail = ownerEmail
	c.Owner = &Employee{
		Company: c,
	...
	} else if c.Owner.Created.UserID == uuid.Nil {
