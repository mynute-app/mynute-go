...
import (
	...
	authHandler "mynute-go/services/auth/handler"
...
func DenyUnauthorized(c *fiber.Ctx) error {
	...
	case namespace.ClientKey.Name:
		userIs = "client"
		user = &model.Client{UserID: claim.ID}
		schema = "public"
	case namespace.EmployeeKey.Name:
		userIs = "employee"
		user = &model.Employee{UserID: claim.ID}
		schema = "company"
	case namespace.AdminKey.Name:
		userIs = "admin"
		user = &model.Admin{UserID: claim.ID}
		schema = "public"
	default:
...
forLoop: // Label is optional but can improve readability
	...
	AuthEngine := authHandler.NewAccessController(tx)
