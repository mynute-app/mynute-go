...
func Test_Permissions(t *testing.T) {
	...
	t.Log("Client tries to create a client: POST /client => 403 (Cannot create other client)")
	http.
		Method("POST").
		URL("/client").
		ExpectStatus(403).
	...
	if len(company1.services) > 1 {
		...
		http.
			...
			ExpectStatus(403).
			Header(namespace.HeadersKey.Auth, employee0AuthToken). // Employee trying to add to himself
			Send(nil)
	} else {
	...
	http.
		...
		ExpectStatus(403).
		Header(namespace.HeadersKey.Auth, employee0AuthToken). // Employee trying action
		Send(nil)
