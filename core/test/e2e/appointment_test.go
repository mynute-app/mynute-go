...
func Test_Appointment(t *testing.T) {
	...
	ct := &modelT.Client{}
	tt.Test(ct.Create(200))
	tt.Test(ct.VerifyEmail(200))
	tt.Test(ct.Login(200))
	tt.Test(ct.Update(200, map[string]any{"name": "Updated client Name"}))
	tt.Test(ct.GetByEmail(200))
	cy := &modelT.Company{}
	...
	tt.Test(err)
	a[0].Create(200, ct.Auth_token, &slot0.StartTimeRFC3339, branchForSlot0, baseEmployee, serviceForSlot0, cy, ct)
	...
	tt.Test(err)
	a[1].Create(200, ct.Auth_token, &slot1.StartTimeRFC3339, branchForSlot1, baseEmployee, serviceForSlot1, cy, ct)
	...
	tt.Test(err)
	a[2].Create(200, cy.Owner.Auth_token, &slot2.StartTimeRFC3339, branchForSlot2, baseEmployee, serviceForSlot2, cy, ct)
	...
	a = append(a, &modelT.Appointment{})
	a[3].Create(409, ct.Auth_token, &startTimeForConflict, branchForSlot0, baseEmployee, serviceForSlot0, cy, ct)
}
