...
func (e *Employee) GetID() string        { return e.Created.UserID.String() }
...
func (e *Employee) Update(s int, changes map[string]any, x_auth_token *string, x_company_id *string) error {
	...
	if err := handler.NewHttpClient().
		Method("PATCH").
		URL(fmt.Sprintf("/employee/%s", e.Created.UserID.String())).
		ExpectedStatus(s).
		Header(namespace.HeadersKey.Company, cID).
		Header(namespace.HeadersKey.Auth, t).
		Send(changes).
...
func (e *Employee) CreateWorkSchedule(s int, EmployeeWorkSchedule DTO.CreateEmployeeWorkSchedule, x_auth_token *string, x_company_id *string) error {
	...
	if err := http.
		Method("POST").
		URL(fmt.Sprintf("/employee/%s/work_schedule", e.Created.UserID.String())).
		ExpectedStatus(s).
		Header(namespace.HeadersKey.Company, cID).
		Header(namespace.HeadersKey.Auth, t).
...
func (e *Employee) UpdateWorkRange(status int, wrID string, changes map[string]any, x_auth_token *string, x_company_id *string) error {
	...
	if err := handler.NewHttpClient().
		Method("PUT").
		URL(fmt.Sprintf("/employee/%s/work_range/%s", e.Created.UserID.String(), wrID)).
...
func (e *Employee) AddServicesToWorkRange(s int, wrID string, body DTO.EmployeeWorkRangeServices, x_auth_token *string, x_company_id *string) error {
	...
	if err := handler.NewHttpClient().
		Method("POST").
		URL(fmt.Sprintf("/employee/%s/work_range/%s/services", e.Created.UserID.String(), wrID)).
		ExpectedStatus(s).
		Header(namespace.HeadersKey.Company, cID).
		Header(namespace.HeadersKey.Auth, t).
		Send(body).
...
func (e *Employee) RemoveServiceFromWorkRange(s int, wrID string, serviceID string, x_auth_token *string, x_company_id *string) error {
	...
	if err := handler.NewHttpClient().
		Method("DELETE").
		URL(fmt.Sprintf("/employee/%s/work_range/%s/service/%s", e.Created.UserID.String(), wrID, serviceID)).
		ExpectedStatus(s).
		Header(namespace.HeadersKey.Company, cID).
		Header(namespace.HeadersKey.Auth, t).
		Send(nil).
...
func (e *Employee) GetByEmail(s int, x_auth_token *string, x_company_id *string) error {
	...
	if err := handler.NewHttpClient().
		Method("GET").
		URL(fmt.Sprintf("/employee/email/%s", url.PathEscape(e.Email))).
		ExpectedStatus(s).
		Header(namespace.HeadersKey.Company, cID).
		Header(namespace.HeadersKey.Auth, t).
		Send(nil).
...
func (e *Employee) LoginWithPassword(s int, x_company_id *string) error {
	if err := e.LoginByPassword(s, e.Password, x_company_id); err != nil {
...
func (e *Employee) SendLoginCode(s int, x_company_id *string) error {
	...
	if err := http.
		Method("POST").
		URL(fmt.Sprintf("/employee/send-login-code/email/%s?lang=en", url.PathEscape(e.Email))).
		ExpectedStatus(s).
		Header(namespace.HeadersKey.Company, cID).
...
func (e *Employee) GetLoginCodeFromEmail() (string, error) {
	...
	if loginMessage == nil {
		return "", fmt.Errorf("no login validation email found for %s", e.Email)
	}
...
func (e *Employee) SendPasswordResetEmail(s int, x_company_id *string) error {
	...
	if err := http.
		Method("POST").
		URL(fmt.Sprintf("/employee/reset-password/%s?lang=en", url.PathEscape(e.Email))).
		ExpectedStatus(s).
		Header(namespace.HeadersKey.Company, cID).
		Send(nil).Error; err != nil {
...
func (e *Employee) SendVerificationEmail(s int, x_company_id *string) error {
	...
	if err := http.
		Method("POST").
		URL(fmt.Sprintf("/employee/send-verification-code/email/%s/%s?language=en", url.PathEscape(e.Email), cID)).
		ExpectedStatus(s).
		Header(namespace.HeadersKey.Company, cID).
		Send(nil).Error; err != nil {
...
func (e *Employee) VerifyEmailByCode(s int, code string, x_company_id *string) error {
	...
	if err := http.
		Method("GET").
		URL(fmt.Sprintf("/employee/verify-email/%s/%s/%s", url.PathEscape(e.Email), code, cID)).
		ExpectedStatus(s).
		Header(namespace.HeadersKey.Company, cID).
		Send(nil).Error; err != nil {
...
// 	if err := handler.NewHttpClient().
// 		Method("POST").
// 		URL(fmt.Sprintf("/employee/verify-email/%s/%s", e.Email, "12345")).
// 		ExpectedStatus(s).
// 		Header(namespace.HeadersKey.Company, cID).
// 		Send(nil).
// 		Error; err != nil {
...
func (e *Employee) AddBranch(s int, b *Branch, token *string, x_company_id *string) error {
	...
	if err := handler.NewHttpClient().
		Method("POST").
		URL(fmt.Sprintf("/employee/%s/branch/%s", e.Created.UserID.String(), b.Created.ID.String())).
		ExpectedStatus(s).
		Header(namespace.HeadersKey.Auth, t).
		Header(namespace.HeadersKey.Company, cID).
		Send(nil).
...
func (e *Employee) AddService(s int, service *Service, token *string, x_company_id *string) error {
	...
	if err := handler.NewHttpClient().
		Method("POST").
		URL(fmt.Sprintf("/employee/%s/service/%s", e.Created.UserID.String(), service.Created.ID.String())).
		ExpectedStatus(s).
		Header(namespace.HeadersKey.Auth, t).
		Header(namespace.HeadersKey.Company, cID).
		Send(nil).
...
func (e *Employee) AddRole(s int, role *Role, x_auth_token *string, x_company_id *string) error {
	...
	if err := handler.NewHttpClient().
		Method("POST").
		URL(fmt.Sprintf("/employee/%s/role/%s", e.Created.UserID.String(), role.Created.ID.String())).
		ExpectedStatus(s).
		Header(namespace.HeadersKey.Auth, t).
		Header(namespace.HeadersKey.Company, cID).
		Send(nil).
...
func (e *Employee) UploadImages(status int, files map[string][]byte, x_auth_token *string, x_company_id *string) error {
	...
	if err := handler.NewHttpClient().
		Method("PATCH").
		URL(fmt.Sprintf("/employee/%s/design/images", e.Created.UserID.String())).
		ExpectedStatus(status).
		Header(namespace.HeadersKey.Auth, t).
		Header(namespace.HeadersKey.Company, cID).
		Send(fileMap).
...
func (e *Employee) DeleteImages(status int, image_types []string, x_auth_token string, x_company_id *string) error {
	...
	base_url := fmt.Sprintf("/employee/%s/design/images", e.Created.UserID.String())
